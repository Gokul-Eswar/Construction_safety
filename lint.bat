...
:: Run clang-tidy
:: --header-filter: Use a regex that matches the current absolute path style
:: We double-escape the backslashes for the regex engine.
clang-tidy -p build !SOURCE_FILES! --extra-arg=--driver-mode=g++ !INCLUDES! --header-filter=".*construction safety.*src.*" > clang_tidy_report.txt 2>&1

echo Analysis Complete. Results saved to clang_tidy_report.txt.
type clang_tidy_report.txt
pause
